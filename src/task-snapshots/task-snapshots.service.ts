...
@Injectable()
export class TaskSnapshotsService {
  ...
  // ─── Refresh: called by TasksService on every task mutation ─────────────
  ...
  async refreshTeamSnapshots(teamId: string): Promise<void> {
    ...
    await Promise.all(
      snapshots.map((snap) =>
        this.prisma.taskSnapshot.upsert({
          where: { key: snap.key },
  ...
  async getTeamVisual(type: VisualType, teamId: string, dto: GetVisualDto) {
    ...
    const snapshot = await this.prisma.taskSnapshot.findUnique({
    ...
    await this.prisma.taskSnapshot.upsert({
