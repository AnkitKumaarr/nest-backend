...
@Injectable()
export class MeetingSnapshotsService {
  ...
  async refreshCompanySnapshots(companyId: string): Promise<void> {
    ...
    await Promise.all(
      snapshots.map((s) =>
        this.prisma.meetingSnapshot.upsert({
          where: { key: s.key },
